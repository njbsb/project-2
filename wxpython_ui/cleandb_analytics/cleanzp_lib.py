...
def arrange_renameCol(df):
    df.rename(columns={'Personnel Number': 'Staff No',
                       ...
                       'Join Div.': 'Date in Division', 'Join Comp.': 'Date in Company',
                       'Join Date': 'Date in PETRONAS', 'Date of Task': 'Retirement Date'}, inplace=True)
    column_order = ['Staff No', 'Staff Name', 'Gender', 'Age', 'SG', 'Lvl', 'JG', 'PPA-19', 'PPA-18', 'PPA-17', 'PPA-16', 'PPA-15', 'PPA',
                    ...
                    'Date in Company', 'Pos. SKG', 'Home SKG', 'Work Center', 'Task Type', 'Task Type Desc', 'Retirement Date']
    df = df[column_order]
    return df
...
def adjustDate(df):
    column_date = ['Date of Birth', 'Join Dept', 'Join Date',
                   'SG Since', 'Date Post', 'Join Div.', 'Join Comp.', 'Date of Task']
    for columnname in column_date:
        df[columnname] = df[columnname].str.replace('.', '/', regex=False)
    return df
...
def addcolumn_age(df):
    ...
    for i, row in df['Date of Birth'].iteritems():
        if not type(row) is float:
            dob = datetime.strptime(row, format_str).date()
...
def set_input_output(opt):
    input_path = r'D:\Documents\Python\project-2\database\input\raw_zpdev_june2020.xlsx' if opt == '1' else r'D:\Documents\Python\project-2\database\input\zpdevc.xlsx'
    output_path = r'D:\Documents\Python\project-2\database\output\zp_full_june.xlsx' if opt == '1' else r'D:\Documents\Python\project-2\database\output\zp_partial.xlsx'
