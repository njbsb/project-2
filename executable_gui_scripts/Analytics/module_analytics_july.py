...
def getDFGroup(df, keyword):
    ...
    print('\nNew Dataframe: {}\nTotal group:'.format(dftype), len(dflist))
    return dflist
...
def addcolumn_superior(df):
    ...
    print('added column superior (2/3)')
    return newdf
...
def addcolumn_consoJG(df):
    ...
    print('added column consoJG (3/3)')
    return df
...
def mainProcess(data_dictionary, operation_type):
    try:
        ...
        logString = 'Process completed: {}'.format(
            logDuration(starttime, time.time()))
        print(logString)
        return df_dictionary, logString
    except Exception as e:
        logString = 'Process failed: {}, {}'.format(str(e),
                                                    logDuration(starttime, time.time()))
        return None, logString


def preProcess(zhplapath, zpdevpath, zpdev_retirepath, operation):
    ...
    if operation == 1:
        ...
        if not zpdev_retirepath == '':
            data_dict['zpdevretire'] = zpdev_retirepath
    if operation == 2:
...
# df_dictionary, logString = mainProcess(data_dictionary, operation)
